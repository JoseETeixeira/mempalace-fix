...
# A non-empty data_level0.bin paired with an absent or zero-byte
# index_metadata.pickle is unambiguously a partial-write segment: chromadb
# 1.5.x buffers up to ``hnsw:sync_threshold`` (PR #1191 raised this to 50_000)
# additions before flushing the pickle, so any process exit before that
# threshold leaves data on disk that the Rust compactor cannot reopen — the
# next ``add``/``upsert`` raises ``Error in compaction: Error constructing
# hnsw segment reader``. We treat ``data_level0.bin > _PARTIAL_DATA_FLOOR`` as
# "real payload exists". The floor (1 KiB) is well below a single 384-dim
# float32 vector (~1.6 KiB) so an empty segment with just a chromadb
# initialization stub never trips the check.
_PARTIAL_DATA_FLOOR = 1024


def quarantine_partial_hnsw_segments(palace_path: str) -> list[str]:
    """Quarantine segment dirs where HNSW payload exists without a usable pickle.

    Recognised partial-write shape:

    * ``data_level0.bin`` has non-trivial size (> ``_PARTIAL_DATA_FLOOR``)
    * ``index_metadata.pickle`` is missing OR zero-byte OR truncated below
      the pickle protocol-2 floor (16 bytes)
    * ``link_lists.bin`` is zero-byte (a real HNSW segment with payload
      always has non-empty link_lists since at least the entry-point node's
      neighbour list is written) — required so we never quarantine a
      mid-flush snapshot whose pickle was simply not yet written but whose
      link_lists is consistent with the data file.

    Renames the entire segment dir to ``<seg>.partial-<timestamp>`` so
    chromadb opens against a fresh empty segment on next call.

    Skipped variants (intentional):

    * ``data_level0.bin`` ≤ floor — fresh / never-flushed segment, nothing
      to recover.
    * Pickle present and parseable — that case is owned by
      :func:`quarantine_invalid_hnsw_metadata` (handles dimensionality-None
      and similar invalid-payload signatures).
    """
    try:
        entries = os.listdir(palace_path)
    except OSError:
    ...
    for name in entries:
        ...
        if data_size <= _PARTIAL_DATA_FLOOR:
            # Fresh / empty segment — nothing to recover.
            continue

        meta_present_and_sized = False
        if os.path.isfile(meta_path):
            try:
                if os.path.getsize(meta_path) >= 16:
                    meta_present_and_sized = True
            except OSError:
                pass
        if meta_present_and_sized:
            # Pickle exists with at least the protocol-2 header — let the
            # invalid-metadata probe (dimensionality check) handle this case.
            continue
        ...
        if os.path.isfile(link_path):
            try:
                ...
                link_size = 0
        if link_size > 0:
            # Mid-flush snapshot whose pickle was simply not yet written but
            # whose link_lists made it to disk. Don't quarantine — chromadb's
            # next flush will catch up.
            continue

        stamp = _dt.datetime.now().strftime("%Y%m%d-%H%M%S")
        target = f"{seg_dir}.partial-{stamp}"
        try:
            os.rename(seg_dir, target)
            moved.append(target)
            logger.warning(
                "Quarantined partial HNSW segment %s (data_level0=%d, link_lists=%d, no pickle); "
                "renamed to %s. ChromaDB will reopen against a fresh empty segment.",
                seg_dir,
                data_size,
                link_size,
                target,
            ...
            logger.exception("Failed to quarantine partial HNSW segment %s", seg_dir)
...
# Substrings that identify chromadb's Rust-side HNSW load failures. The
# Rust bindings wrap these in ``chromadb.errors.InternalError`` (and a few
# subclasses depending on the entry path). We match by message because the
# exception class hierarchy varies across chromadb versions.
_HNSW_RUNTIME_LOAD_MARKERS = (
...
def _is_hnsw_runtime_load_error(exc: BaseException) -> bool:
    """Detect chromadb Rust-side HNSW load failures by exception message.

    Mirrors the pre-flight divergence check at runtime: the pre-flight only
    catches "sqlite says N, HNSW pickle says M" mismatches, while this fires
    when chromadb's compactor refuses to map a segment even though the
    pickle counts looked fine (observed after `mempalace repair` produces
    a freshly-rebuilt segment).
    """
    msg = str(exc)
    return any(marker in msg for marker in _HNSW_RUNTIME_LOAD_MARKERS)
...
class ChromaCollection(BaseCollection):
    ...
    When ``palace_path`` and ``collection_name`` are both set, ``count``,
    ``get``, and ``query`` catch the chromadb HNSW-load Rust failure
    (``_HNSW_RUNTIME_LOAD_MARKERS``) and promote this collection to a
    :class:`mempalace.backends.sqlite_fallback.SqliteReadOnlyCollection`
    for the rest of its life. Subsequent reads bypass chromadb entirely;
    writes and ``query`` raise ``CollectionDegradedError`` with a clear
    repair hint instead of repeatedly hitting the same Rust error.
    """
    ...
    def _maybe_fallback(self):
        """Return a lazy sqlite-backed fallback collection.

        Returns ``None`` when palace context is missing — older callers
        that constructed ChromaCollection directly without going through
        ChromaBackend lose only the runtime-fallback safety net. Static
        divergence detection still applies because it runs in
        ``ChromaBackend.get_collection``.
        """
        if self._palace_path is None or self._collection_name is None:
            return None
        if self._sqlite_fallback is not None:
            return self._sqlite_fallback
        from .sqlite_fallback import (
            SqliteReadOnlyCollection,
            _DIVERGENCE_CACHE,
            _DIVERGENCE_LOCK,
        )

        # Promote this (palace, collection) to "diverged" in the cache so
        # the next ``get_collection`` call short-circuits to sqlite without
        # spending a chromadb round-trip first.
        with _DIVERGENCE_LOCK:
            _DIVERGENCE_CACHE[(self._palace_path, self._collection_name)] = True
        self._sqlite_fallback = SqliteReadOnlyCollection(
            self._palace_path,
            self._collection_name,
            "chromadb cannot load HNSW segment (runtime fallback)",
        )
        return self._sqlite_fallback
    ...
    def query(
        ...
        *,
        ...
        try:
            ...
            if _is_hnsw_runtime_load_error(exc):
                fallback = self._maybe_fallback()
                if fallback is not None:
                    # Promote so future reads skip chromadb, then re-raise as
                    # CollectionDegradedError so callers (CLI search, MCP
                    # search) auto-route to the BM25-sqlite fallback path
                    # rather than silently get an empty result.
                    logger.warning(
                        "ChromaDB query() failed (%s); marking '%s' as runtime-"
                        "diverged. Future reads will use sqlite fallback; "
                        "semantic search remains unavailable until repair.",
                        exc,
                        self._collection_name,
                    )
                    fallback.query()  # raises CollectionDegradedError with hint
            raise
    ...
    def get(
        ...
        *,
        ...
        # Skip chromadb entirely once a prior call promoted this collection
        # to the sqlite fallback.
        if self._sqlite_fallback is not None:
            return self._sqlite_fallback.get(
                ids=ids,
                where=where,
        ...
        except Exception as exc:  # noqa: BLE001 — chromadb wraps Rust errors in many classes
            if _is_hnsw_runtime_load_error(exc):
                fallback = self._maybe_fallback()
                if fallback is not None:
                    logger.warning(
                        "ChromaDB HNSW load failed (%s); falling back to sqlite read for "
                        "'%s' going forward",
                        exc,
                        self._collection_name,
                    )
    ...
    def count(self):
        if self._sqlite_fallback is not None:
            return self._sqlite_fallback.count()
        try:
            ...
            if _is_hnsw_runtime_load_error(exc):
                ...
                if fallback is not None:
                    logger.warning(
                        "ChromaDB count() failed (%s); falling back to sqlite for "
                        "'%s' going forward",
                        exc,
                        self._collection_name,
                    )
    ...
    * An inode+mtime freshness check absorbed from ``mcp_server._get_client``
    ...
    def _prepare_palace_for_open(palace_path: str) -> None:
        ...
        if palace_path not in ChromaBackend._quarantined_paths:
            ...
            # Partial-write segments (payload-without-pickle, link_lists empty)
            # break the Rust compactor on the very next add/upsert — exactly
            # the "save FAIL — HNSW index broken" failure mode observed when a
            # rebuild process exited before chromadb's pickle flush. Catching
            # it here means every cold-start opens against either a healthy
            # segment or a clean empty one, never against a half-written one.
            quarantine_partial_hnsw_segments(palace_path)
    ...
    def get_collection(
        ...
        * Legacy: ``get_collection(palace_path, collection_name, create=False)``
        ...
        # Static divergence guard. The pre-flight probe reads sqlite + the
        # HNSW metadata pickle directly and never hits chromadb's Rust
        # binding — so even if the segment would segfault on count()/get(),
        # we route reads through SqliteReadOnlyCollection instead of touching
        # chromadb. Skip when create=True (caller is asking to recover) or
        # when the palace database does not exist yet (first init).
        if not create and os.path.isfile(os.path.join(palace_path, "chroma.sqlite3")):
            from .sqlite_fallback import (
                SqliteReadOnlyCollection,
                is_critically_diverged,
            )

            status = is_critically_diverged(palace_path, collection_name)
            if status is not None:
                msg = status.get("message") or "HNSW index is divergent from sqlite"
                logger.warning(
                    "Routing '%s' through sqlite fallback: %s",
                    collection_name,
                    msg,
                )
                return SqliteReadOnlyCollection(palace_path, collection_name, msg)
        ...
        return ChromaCollection(
            collection, palace_path=palace_path, collection_name=collection_name
        )
    ...
    def close(self) -> None:
        ...
        self._closed = True
        # Drop per-process divergence verdicts too so tests / repair tooling
        # that calls ``close()`` to reset state start clean.
        try:
            from .sqlite_fallback import invalidate_divergence_cache

            invalidate_divergence_cache()
        except Exception:
            logger.debug("invalidate_divergence_cache failed on close", exc_info=True)
    ...
    def delete_collection(self, palace_path: str, collection_name: str) -> None:
        ...
        # A delete invalidates any cached divergence verdict — the next
        # ``get_collection`` should probe the freshly-rebuilt segment.
        try:
            from .sqlite_fallback import invalidate_divergence_cache

            invalidate_divergence_cache(palace_path)
        except Exception:
            logger.debug("invalidate_divergence_cache failed after delete", exc_info=True)
