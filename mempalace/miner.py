...
def status(palace_path: str):
    ...
    try:
        ...
        fix_summary = auto_fix_palace(palace_path)
        for line in render_summary(fix_summary):
            print(line)
    except Exception:
        # Defensive: autofix must never block status. Real failures are
        # captured inside the per-fix reports; this catches a top-level
        # import or attribute error.
        logger.debug("status autofix raised", exc_info=True)
