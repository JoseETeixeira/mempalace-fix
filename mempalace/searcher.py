...
def _print_bm25_results(query: str, wing, room, bm25_result: dict) -> None:
    """Format and print the BM25-sqlite fallback in the same layout as the vector path.

    Used by :func:`search` when the vector path is unavailable so the user
    sees results instead of an error and a repair hint.
    """
    hits = bm25_result.get("results") or []
    print(f"\n{'=' * 60}")
    print(f'  Results for: "{query}"  (BM25 sqlite fallback)')
    if wing:
        print(f"  Wing: {wing}")
    if room:
        print(f"  Room: {room}")
    ...
    for i, hit in enumerate(hits, 1):
        ...
        print(f"      Match:  bm25={bm25}  (vector unavailable)")
        print()
...
def search(query: str, palace_path: str, wing: str = None, room: str = None, n_results: int = 5):
    ...
    When chromadb's HNSW segment is unavailable (the #1222 failure mode
    plus its post-repair sibling), auto-falls back to BM25 over
    ``chroma.sqlite3`` so the user gets results instead of an error.
    """
    from .backends.sqlite_fallback import (
        CollectionDegradedError,
        SqliteReadOnlyCollection as _SqliteReadOnlyCollection,
    )
    ...
    backend_degraded = isinstance(col, _SqliteReadOnlyCollection)
    if not backend_degraded:
    ...
    if not backend_degraded:
        ...
        except CollectionDegradedError as e:
            print(f"\n  Vector search unavailable ({e})")
            print("  Falling back to BM25 over chroma.sqlite3 ...")
    ...
    if backend_degraded:
        bm25_result = _bm25_only_via_sqlite(
            query,
            palace_path,
            wing=wing,
            room=room,
        ...
        if "error" in bm25_result:
            print(f"\n  Search error: {bm25_result['error']}")
            raise SearchError(bm25_result["error"])
        return _print_bm25_results(query, wing, room, bm25_result)
...
def search_memories(
    ...
    try:
        ...
        # Runtime HNSW load failure where the static probe missed it.
        # Auto-route to the BM25-sqlite path the vector_disabled flag
        # already supports so callers get results instead of an error.
        from .backends.sqlite_fallback import CollectionDegradedError

        if isinstance(e, CollectionDegradedError) or any(
            marker in str(e)
            for marker in (
            ...
            logger.warning(
                "search_memories: vector path unavailable (%s); auto-routing to BM25 sqlite",
                e,
            )
            ...
            result["auto_fallback"] = "bm25_sqlite"
            return result
